...
const getUsers = async (req: Request, res: Response) => {
	try {
		...
		if (!candidates.length) return successResponse(res, `No candidate available!`, []);
		return successResponse(res, `${candidates.length} candidate${candidates.length > 1 ? 's' : ''} retrived!`, candidates);
	} catch (error) {
		console.log(error);
		return handleResponse(res, 401, false, `An error occured - ${error}`);
	}
...
const getUserDetails = async (req: Request, res: Response) => {
	...
	const { id } = req.params;
	try {
		const candidate = await DB.candidates.findOne({ where: { id }, attributes: { exclude: ['password'] }, include: { model: DB.documents } });
		if (!candidate) return errorResponse(res, `Candidate with ID ${id} not found!`);
		return successResponse(res, `Candidate details retrived!`, candidate);
	} catch (error) {
		console.log(error);
		return handleResponse(res, 401, false, `An error occured - ${error}`);
	}
